...
export async function updateProjectTask(id: string, data: unknown) {
  ...
  try {
    ...
    const desc = statusChanged && newStatus === "done"
...
export async function toggleProjectMilestone(id: string) {
  try {
    ...
    const [row] = await db
      ...
      .where(eq(projectMilestones.id, milestoneId))
      .returning({ id: projectMilestones.id, title: projectMilestones.title, projectId: projectMilestones.projectId, isCompleted: projectMilestones.isCompleted });
